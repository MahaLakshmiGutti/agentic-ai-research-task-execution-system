...
This project is not yet deployed. To deploy it:
...
5. Add environment variables in Render's dashboard: `GEMINI_API_KEY`,
   `GEMINI_MODEL`, `TAVILY_API_KEY`, `CORS_ORIGINS` (set this to your Vercel
   frontend's URL once you have it), `DATABASE_PATH`.
...
3. Since there is no Vite dev proxy in production, either:
   - add a `vercel.json` rewrite that proxies `/api/*` to your Render backend
     URL, or
   - update `frontend/src/api.ts`'s `BASE` constant to the full Render
     backend URL (e.g. via a `VITE_API_BASE` build-time env var) instead of
...
- **No RAG / vector database.** Research is done purely via live Tavily web
  search, not a vector store over documents.
- **No document/PDF ingestion.**
- **SQLite, not Postgres/Mongo/Redis.** Chosen for a zero-infrastructure
  local MVP; swap the `sqlite:///` URL built from `DATABASE_PATH` in
  `app/config.py`'s `database_url` property for a Postgres URL if you need
  multi-instance/production durability.
- **No conversational memory.** Each run is fully independent — the Planner
  has no awareness of previous runs' results, even though those results are
  durably stored and browsable in the history sidebar. Adding a
  `conversation_id`/`parent_run_id` to seed a new run with a prior run's
  report as extra context would close this gap.
- **Not yet deployed** to Render/Vercel — see [Deployment](#deployment) above
  for how to do it.
